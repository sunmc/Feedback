	...
	<div id="offCanvasWrapper" class="mui-off-canvas-wrap mui-draggable">
		...
		<div class="mui-inner-wrap">
			<header class="mui-bar mui-bar-nav">
				...
				<h1 class="mui-title"><%= Common.wtcl + Common.lb%></h1>
			</header>
	...
	<script src="/Feedback/resource/js/jquery1.8.0.min.js"></script>
		<script>
			...
			function pulldownRefresh() {
				...
				$.post("/Feedback/wtcl/wtcllist.do",
				         function(data){
							if(data.flag){
								var wtcldata = data.data;
								var urls = new Array(); 
								$("#itemlist").empty();
								for(var i = 0; i < wtcldata.length; i++){
									var li = '<li id='+ i +' class="mui-table-view-cell mui-media wi">'+
												'<a class="mui-navigate-right">' + 
												wtcldata[i].lsh + ' ' +  wtcldata[i].xmbh + ' ' + wtcldata[i].wtms +
													'<p class="mui-ellipsis">' + wtcldata[i].fqrxm + ' ' + wtcldata[i].createtimes + '</p>' +
												'</a>' +
												'<div hidden="hidden">' +
													'<input type="text" id="objectid' + i + '" value="' + wtcldata[i].objectid + '">' + 
													'<input type="text" id="workitemid' + i + '" value="' + wtcldata[i].workitemid + '">' + 
												'</div>' +
											'</li>';
											
									//下拉刷新，新纪录插到最前面；
									$("#itemlist").prepend(li);
									urls[i] = '/Feedback/wtcl/wtcl.do?objectid=' + wtcldata[i].objectid + '&workitemid=' + wtcldata[i].workitemid;
